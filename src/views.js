...
const generateRecipeToDOM = recipe => {
  ...
  const ingredientsWeHave = recipe.ingredients.filter(
    ingredient => ingredient.itHas === true
  );
  const ingredientsCounter =
    ingredientsWeHave.length !== recipe.ingredients.length
      ? `${ingredientsWeHave.length} of ${recipe.ingredients.length} ingredients`
      : "All ingredients";
  ...
  recipeTime.innerHTML = `<i class="fas fa-clock"></i> ${recipe.time ||
    "0 Mins."}`;
  recipeTime.classList.add("recipe-time");
  recipeInfo.appendChild(recipeTime);

  recipeDifficulty.innerHTML = `<i class="fas fa-smile"></i> ${recipe.difficulty ||
    "Easy"}`;
  recipeDifficulty.classList.add("recipe-difficulty");
  recipeInfo.appendChild(recipeDifficulty);

  recipeLikes.innerHTML = `<i class="fas fa-heart"></i> ${recipe.likes || "0"}`;
